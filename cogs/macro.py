...
class Macro(commands.Cog):
    ...
    async def send(self, ctx, file: str, *, message: str) -> None:
            ...
            for channel_id in ids:
                ...
                if channel:
                    try:
                        ...
                        queue_status.append(
                            f"- ``{channel.guild.name}`` ﹕ {channel.mention}     ⸝⸝⸝     ``429 WARNING``")
                    except discord.Forbidden:
                        queue_status.append(
                            f"- ``{channel.guild.name}`` ﹕ {channel.mention}     ⸝⸝⸝     ``403 WARNING``")
                    except discord.HTTPException as e:
                        queue_status.append(
                            f"- ``{channel.guild.name}`` ﹕ {channel.mention}     ⸝⸝⸝     ``401 WARNING``")
                else:
        ...
        queue_status.append(
            f"-# finished executing at {ctx.message.created_at}")
        await status_message.edit(content="\n".join(queue_status))
    ...
    async def check(self, ctx, file: str) -> None:
        ...
        try:
            ...
            for line in raw_data:
                ...
                if " | " in line:
                    parts = line.split(" | ")
                    if len(parts) == 2 and all(part.isdigit()
                                               for part in parts):
                ...
                for line in structured_lines:
                    try:
                        ...
                        if channel:
                            ...
                            # await ctx.send(f"- ``{channel.guild.name}`` ﹕
                            # {channel.mention}     ⸝⸝⸝     ``200 OK``")
                        else:
                            invalid_channels.append(line)
    ...
    async def fetch_all(self, ctx, file: str) -> None:
        ...
        try:
            ...
            with open(file_path, "a") as promo_file:
                try:
                    existing_channels = {int(line.strip().split(
                        " | ")[1]) for line in open(file_path).readlines()}
                except FileNotFoundError:
